...
export default function AddDoctorForm({ sendMessage, data, handleRefresh }) {
  ...
  return (
      ...
      onFinish={async () => {
        if (data) {
          try {
            ...
            const result = await API.CRUD.updateItem(form.getFieldsValue(),"doctor",data.iddoctor);
            // console.log("result update doctor: ",result);
            handleRefresh();
          } catch (error) {
          ...
          try {
            const result = await API.CRUD.addItem(form.getFieldsValue(),"doctor");
    ...
    >
      <Form.Item
        name="name"
        label="doctor name"
        rules={[{ required: true, message: "please input your doctor name" }]}
      >
        <Input></Input>
      </Form.Item>
      <Row>
      ...
      <Form.Item
        name="cmd"
        label="cmd"
        rules={[{ required: true, message: "please input doctor cmd" }]}
      >
        <Input></Input>
      </Form.Item>
      <Form.Item
        name="address"
        label="address"
        rules={[{ required: true, message: "please input doctor address" }]}
      >
        <Input></Input>
      </Form.Item>
      <Form.Item
        name="seniority"
        label="seniority"
        rules={[{ required: true, message: "please input doctor seniority" }]}
      >
